...
module.exports.sockets = {
  ...
  ***************************************************************************/
  afterDisconnect: function(session, socket, cb) {
    ...
    NotifyOnline.destroy({socketId:socket.id}).exec(function(err,notification){
      if (err) return cb();
      if (notification.length) {
                sails.sockets.leave(socket, notification[0].roomName);
                console.log("Destroy Notification Socket: ", socket.id);
                console.log(notification[0].userName + " left the notification room.");
      }

      try {
          //var userId = session.users[sails.sockets.id(socket)].id;
            // Destroy the user instance
            Userchat.destroy({socketId:socket.id}).exec(function(err,user){

              if (err) return cb();
              if (user.length) {
                sails.sockets.leave(socket, user[0].refund_num);
                console.log("Destroy Chat Socket: ", socket.id);
                console.log(user[0].nickName + " left the chat room.");
              }
              return cb();          
            });
        } catch (e) {
